...
def list_of_real_predicted_new_cases(sat, scenario, gf, plot_days, smooth=True):
    
    
    # worst = 0.11872268  ## INPUT
    worst = np.log10(gf)
    medium = worst / 2  ## INPUT
    best = worst / 3  ## INPUT
    plot_days = plot_days  ## INPUT length of plot
    
    
    # Define days since measures are taken
    day_of_measures = '16-03-2020'
    today = dt.date.today()
    days_since = (pd.to_datetime(today) - pd.to_datetime(day_of_measures)).days + 1
    
    
    # create space of growth factors growing with sigmoid function
    if scenario == 'best':
        sat = np.linspace(worst,0,sat[0])
        sat = np.pad(sat, (0, 100), 'constant')       # add zeros
        # pick GFs from days since measures untill days to plot
        range_gf = sat[days_since:plot_days+days_since+1]
    elif scenario == 'medium':
        sat = np.linspace(worst,0,sat[1])
        sat = np.pad(sat, (0, 100), 'constant')       # add zeros
        # pick GFs from days since measures untill days to plot
        range_gf = sat[days_since:plot_days+days_since+1]
    elif scenario == 'worst':
        sat = np.linspace(worst,0,sat[2])
        sat = np.pad(sat, (0, 100), 'constant')       # add zeros
        # pick GFs from days since measures untill days to plot
        range_gf = sat[days_since:plot_days+days_since+1]
    ...
    scenarios = {'worst': [{'duration': plot_days, 'growth': 10*worst}],
            'medium': [{'duration': plot_days, 'growth': 10*worst}],
            'best': [{'duration': plot_days, 'growth': 10*worst}]}
...
def plot_future(df_ic, range_gf, pivots=[{'duration': 10, 'growth': 1.25},
    ...
    for pivot_dict in pivots:
        ...
        new_values = []
        for i in range(pivot_dict['duration']):
            new_value = last_value*(10**range_gf[i])
    ...
    df_ic['date'] = pd.date_range(start=first_date, end=first_date + np.timedelta64(days + total_predicted_days - 1, 'D'))
                      # dt.timedelta(days=days + total_predicted_days - 1))
